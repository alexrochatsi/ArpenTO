...
public class CadastroIbgeUF extends CadastroPadrao{
   ...
   @SuppressWarnings("unchecked")
   public List<IbgeUF> listaIbgeUFsPorCidade(String cidade) throws ApplicationException, CadastroException {
	  Session session=null;
	  GerenteTransacao gerenteTransacao=getContexto().getFabricaGerente().getGerenteTransacao();
	  session=(Session) gerenteTransacao.getSessaoAtual();
	  try {
		 Query query=session.createQuery("from IbgeUF u where u.nomeMunicipio like :cidade");
   ...
   public List<IbgeUF> listaIbgeUFsPorUF(String uf) throws ApplicationException, CadastroException {
	  Session session=null;
	  GerenteTransacao gerenteTransacao=getContexto().getFabricaGerente().getGerenteTransacao();
	  session=(Session) gerenteTransacao.getSessaoAtual();
	  try {
		 Query query=session.createQuery("from IbgeUF u where u.uf like :uf");
		 query.setString("uf" , uf.toUpperCase());
