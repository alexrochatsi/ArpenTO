...
public class CadastroCartorio extends CadastroPadrao{
   ...
   public Cartorio findByUsuarioCatorioId(UsuarioCartorio usuarioCartorio) throws CadastroException, FinderException {
	  Session session=null;
	  try {
		 GerenteTransacao gerenteTransacao=getContexto().getFabricaGerente().getGerenteTransacao();
		 session=(Session) gerenteTransacao.getSessaoAtual();
		 ...
		 Query query=session.createQuery("from Cartorio c where c.id = :id ");
		 query.setInteger("id" , usuarioCartorio.getCartorio().getId());
   ...
   public List<Cartorio> findAll() throws CadastroException {
	  ...
	  try {
		 ...
		 Query query=session.createQuery("select c.id, c.nome, c.municipio from Cartorio c order by c.nome");
   ...
   @SuppressWarnings("unchecked")
   public List<Cartorio> findByNomeMunicipio() throws CadastroException {
	  Session session=null;
	  try {
		 GerenteTransacao gerenteTransacao=getContexto().getFabricaGerente().getGerenteTransacao();
		 session=(Session) gerenteTransacao.getSessaoAtual();

		 Criteria c=session.createCriteria(Cartorio.class , "a");

		 ProjectionList proList=Projections.projectionList();
		 proList.add(Projections.property("a.id").as("id"));
		 proList.add(Projections.property("a.nome").as("nome"));
		 proList.add(Projections.property("a.municipio").as("municipio"));
		 c.setProjection(proList).setResultTransformer(new AliasToBeanResultTransformer(Cartorio.class));
